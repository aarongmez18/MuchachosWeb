...
export class ProcesionalComponent implements OnInit{
  repertoriosProcesional: Marcha[] = [];
  errorMessage = '';

  constructor(private readonly marchaService: MarchaService, private readonly router: Router) {}

  ngOnInit(): void {
    this.loadProcesionalRepertorios();
  }

  loadProcesionalRepertorios(): void {
    this.marchaService.getAllMarchas().subscribe({
      next: (data: Marcha[]) => {
        // Filtramos solo los que tienen código "PROCESIONAL"
        this.repertoriosProcesional = data.filter(rep => rep.codigoRepertorio === 'PROCESIONAL');
      },
      error: (err) => {
